...
export default function Index() {
  ...
  const pickImage = async () => {
    try {
      ...
      Alert.alert(
        "选择提示", 
        `请选择不超过${MAX_IMAGES}张图片，超过部分将被忽略`,
      ...
      if (!result.canceled) {
        ...
        // TODO: 这里添加处理图片的逻辑，如上传到服务器或本地分析
        Alert.alert("成功", `已选择${result.assets.length}张图片，正在分析中...`);
  ...
  return (
    ...
    <View className="flex-1 items-center justify-center bg-gray-100 p-4">
      ...
      <Link href="/chat-bot" className="text-blue-500 p-2">ChatBot</Link>
    </View>
  );
} 